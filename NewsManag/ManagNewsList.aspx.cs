	...
	public partial class ManagNewsList : System.Web.UI.Page
	{
		...
		protected void Page_Load(object sender, System.EventArgs e)
		{
			...
			if (!IsPostBack)
			{
				...
				else
				{
                    ...
                    ButIssuNews.NavigateUrl = "IssuNews.aspx";
                    ButIssuNews.Target = "main";
		...
		private void DataGridNews_DeleteCommand(object source, System.Web.UI.WebControls.DataGridCommandEventArgs e)
		{
			...
			if ((myLoginID.Trim().ToUpper()=="ADMIN")||(myLoginID.Trim().ToUpper()==e.Item.Cells[5].Text.Trim().ToUpper()))
			{
                ...
                AccessDateHelper.ExecuteNonQuery("delete from NewsInfo where NewsID=" + intNewsID);
		...
		private void ShowData(string strSql)
		{
			...
			{				
                ...
                HyperLink LBEditNews = (HyperLink)DataGridNews.Items[i].FindControl("LinkButEditNews");
				...
				if ((myLoginID.Trim().ToUpper()=="ADMIN")||(myLoginID.Trim().ToUpper()==DataGridNews.Items[i].Cells[5].Text.Trim().ToUpper()))
				{
                    ...
                    LBEditNews.NavigateUrl = "EditNews.aspx?NewsID=" + DataGridNews.Items[i].Cells[0].Text + "";
                    LBEditNews.Target = "main";
			...
			if (Request["chkSelect"]!=null)
			{
				...
				for (int j=0;j<textArray.Length;j++)
			    {
					...
					if ((myLoginID.Trim().ToUpper()=="ADMIN")||(myLoginID.Trim().ToUpper()==DataGridNews.Items[i].Cells[5].Text.Trim().ToUpper()))
					{
                        ...
                        //SqlCmd.ExecuteNonQuery();
                        AccessDateHelper.ExecuteNonQuery("delete from NewsInfo where NewsID=" + DataGridNews.Items[i].Cells[0].Text.Trim());

                        //SqlCmd=new SqlCommand("delete from NewsUser where NewsID="+DataGridNews.Items[i].Cells[0].Text.Trim(),SqlConn);
                        //SqlCmd.ExecuteNonQuery();

                        AccessDateHelper.ExecuteNonQuery("delete from NewsUser where NewsID=" + DataGridNews.Items[i].Cells[0].Text.Trim());
					}
				}
				//SqlConn.Close();
		...
		{
			bWhere=false;
			strSql="select a.NewsID,a.NewsTitle,a.NewsContent,a.BrowNumber,b.LoginID as CreateLoginID,a.CreateUserID,a.CreateDate as CreateDate from NewsInfo a LEFT OUTER JOIN UserInfo b ON a.CreateUserID=b.UserID";
			//显示以新闻标题为条件的数据
