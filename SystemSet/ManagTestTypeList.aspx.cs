	...
	public partial class ManagTestTypeList : System.Web.UI.Page
	{
		...
		private void ShowData(string strSql)
		{
			...
			for(int i=0;i<DataGridTestType.Items.Count;i++)
			{
                ...
                if (AccessDateHelper.GetValues("select TestTypeID from RubricInfo where TestTypeID=" + DataGridTestType.Items[i].Cells[0].Text.Trim() + "", "TestTypeID") != "")
				{
		...
		protected void ButNewDept_Click(object sender, System.EventArgs e)
		{
            ...
            string strTmp = AccessDateHelper.GetValues("select TestTypeName from TestTypeInfo where TestTypeName='" + ObjFun.getStr(ObjFun.CheckString(txtTestTypeName.Text.Trim()), 20) + "'", "TestTypeName");
			if (strTmp=="")
			{
                //string strConn=ConfigurationSettings.AppSettings["strConn"];
                //SqlConnection SqlConn=new SqlConnection(strConn);
		...
		private void DataGridTestType_UpdateCommand(object source, System.Web.UI.WebControls.DataGridCommandEventArgs e)
		{
            ...
            string strTmp = AccessDateHelper.GetValues("select TestTypeName from TestTypeInfo where TestTypeName='" + ObjFun.getStr(ObjFun.CheckString(strName.Trim()), 20) + "'and TestTypeID<>" + intTestTypeID + "", "TestTypeName");
			if (strTmp=="")
			{
                ...
                //SqlConn.Close();
                //SqlConn.Dispose();
                AccessDateHelper.ExecuteNonQuery("update TestTypeInfo set TestTypeName='" + ObjFun.getStr(ObjFun.CheckString(strName.Trim()), 20) + "',BaseTestType='" + strBaseTestType + "' where TestTypeID=" + intTestTypeID);
			}
			else
			{
				this.RegisterStartupScript("newWindow","<script language='javascript'>alert('此题型名称已经存在！')</script>");
		...
		private void DataGridTestType_DeleteCommand(object source, System.Web.UI.WebControls.DataGridCommandEventArgs e)
		{
            ...
            if (AccessDateHelper.GetValues("select TestTypeID from RubricInfo where TestTypeID=" + intTestTypeID + "", "TestTypeID") != "")
			{
				this.RegisterStartupScript("newWindow","<script language='javascript'>alert('此题型正在使用，请先删除题库中相应试题后再进行此操作！')</script>");
				return;
			}
			else
			{
                ...
                //SqlConn.Close();
                //SqlConn.Dispose();
                AccessDateHelper.ExecuteNonQuery("delete from TestTypeInfo where TestTypeID=" + intTestTypeID + "");
			...
			if (Request["chkSelect"]!=null)
			{
				...
				for (int j=0;j<textArray.Length;j++)
				{
					...
					if (ObjFun.GetValues("select TestTypeID from RubricInfo where TestTypeID="+DataGridTestType.Items[i].Cells[0].Text.Trim()+"","TestTypeID")=="")
					{
                        ...
                        //SqlCmd.ExecuteNonQuery();
                        AccessDateHelper.ExecuteNonQuery("delete from TestTypeInfo where TestTypeID=" + DataGridTestType.Items[i].Cells[0].Text.Trim());
					}
				}
