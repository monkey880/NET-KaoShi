	...
	public partial class ManagPowerList : System.Web.UI.Page
	{
		...
		protected void Page_Load(object sender, System.EventArgs e)
		{
			...
			if (!IsPostBack)
			{
				...
				{
                    strSql = "select a.UserID,a.LoginID,a.UserName,a.UserSex,b.DeptName,c.JobName,switch( a.UserState = 1 , '正常' ,a.UserState = 0 , '禁止' ) as UserState from (UserInfo a LEFT  JOIN DeptInfo b ON a.DeptID = b.DeptID) LEFT  JOIN JobInfo c ON a.JobID = c.JobID where a.UserType=1 and LoginID<>'Admin' order by a.LoginID desc";
...
//				}
		...
		{
			bWhere=true;
            strSql = "select a.UserID,a.LoginID,a.UserName,a.UserSex,b.DeptName,c.JobName,switch( a.UserState = 1 , '正常' ,a.UserState = 0 , '禁止' ) as UserState from (UserInfo a LEFT OUTER JOIN DeptInfo b ON a.DeptID = b.DeptID) LEFT OUTER JOIN JobInfo c ON a.JobID = c.JobID where a.UserType=1 and LoginID<>'Admin'";
			//显示以部门为条件的数据
