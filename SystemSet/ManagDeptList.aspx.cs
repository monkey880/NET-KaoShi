...
namespace EasyExam.SystemSet
{
	...
	public partial class ManagDeptList : System.Web.UI.Page
	{
		...
		protected void Page_Load(object sender, System.EventArgs e)
		{
			...
			if (!IsPostBack)
			{
				...
				else
				{
					...
					ButDelete.Attributes.Add("onclick","javascript:{if(confirm('确实要删除选择群组吗？')==false) return false;}");
		...
		protected void ButNewDept_Click(object sender, System.EventArgs e)
		{
			...
			{
				this.RegisterStartupScript("newWindow","<script language='javascript'>alert('群组名称不能为空！')</script>");
				return;
			}
			...
			if (strTmp=="")
			{
                ...
                //SqlConn.Close();
                //SqlConn.Dispose();
                AccessDateHelper.ExecuteNonQuery("Insert into DeptInfo(DeptName) values('" + ObjFun.getStr(ObjFun.CheckString(txtDeptName.Text.Trim()), 20) + "')");
				this.RegisterStartupScript("newWindow","<script language='javascript'>alert('新建群组名称成功！')</script>");
			}
			else
			{
				this.RegisterStartupScript("newWindow","<script language='javascript'>alert('此群组名称已经存在！')</script>");
				return;
			}
			txtDeptName.Text="";
			ShowData(strSql);
		...
		private void DataGridDept_DeleteCommand(object source, System.Web.UI.WebControls.DataGridCommandEventArgs e)
		{
            ...
            if (AccessDateHelper.GetValues("select UserID from UserInfo where DeptID=" + intDeptID + "", "UserID") == "")
			{
                //string strConn=ConfigurationSettings.AppSettings["strConn"];
                //SqlConnection SqlConn=new SqlConnection(strConn);
                //SqlConn.Open();
                ...
                //SqlConn.Close();
                //SqlConn.Dispose();
                AccessDateHelper.ExecuteNonQuery("delete from DeptInfo where DeptID=" + intDeptID + "");
			...
			{
				this.RegisterStartupScript("newWindow","<script language='javascript'>alert('此群组正在使用，请先删除帐户中相应帐户后再进行此操作！')</script>");
				return;
			}
		}
		#endregion
		...
		private void DataGridDept_UpdateCommand(object source, System.Web.UI.WebControls.DataGridCommandEventArgs e)
		{
            ...
            string strTmp = AccessDateHelper.GetValues("select DeptName from DeptInfo where DeptName='" + ObjFun.getStr(ObjFun.CheckString(strName.Trim()), 20) + "'and DeptID<>" + intDeptID + "", "DeptName");
			if (strTmp=="")
			{
                //string strConn=ConfigurationSettings.AppSettings["strConn"];
                //SqlConnection SqlConn=new SqlConnection(strConn);
                ...
                AccessDateHelper.ExecuteNonQuery("update DeptInfo set DeptName='" + ObjFun.getStr(ObjFun.CheckString(strName.Trim()), 20) + "' where DeptID=" + intDeptID + "");
			}
			else
			{
				this.RegisterStartupScript("newWindow","<script language='javascript'>alert('此群组名称已经存在！')</script>");
		...
		private void ShowData(string strSql)
		{
			...
			DataGridDept.DataSource=SqlDS.Tables[0].DefaultView;
			...
			{	
				...
				LBDel.Attributes.Add("onclick", "javascript:{if(confirm('确定要删除选择群组吗？')==false) return false;}");
			}
		...
		protected void ButDelete_Click(object sender, System.EventArgs e)
		{
			if (Request["chkSelect"]!=null)
			{
				...
				for (int j=0;j<textArray.Length;j++)
				{
                    ...
                    if (AccessDateHelper.GetValues("select UserID from UserInfo where DeptID='" + DataGridDept.Items[i].Cells[0].Text.Trim() + "'", "UserID") == "")
					{
                        //SqlCmd=new SqlCommand("delete from DeptInfo where DeptID="+DataGridDept.Items[i].Cells[0].Text.Trim(),SqlConn);
                        //SqlCmd.ExecuteNonQuery();
                        AccessDateHelper.ExecuteNonQuery("delete from DeptInfo where DeptID=" + DataGridDept.Items[i].Cells[0].Text.Trim());
					}
				}
