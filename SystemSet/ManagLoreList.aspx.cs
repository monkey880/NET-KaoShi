...
namespace EasyExam.SystemSet
{
	...
	public partial class ManagLoreList : System.Web.UI.Page
	{
		...
		private void ShowData(string strSql)
		{
			...
			DataGridLore.DataSource=SqlDS.Tables[0].DefaultView;
		...
		protected void ButNewDept_Click(object sender, System.EventArgs e)
		{
			if (intSubjectID!=0)
			{
                ...
                string strTmp = AccessDateHelper.GetValues("select LoreName from LoreInfo where LoreName='" + ObjFun.getStr(ObjFun.CheckString(txtLore.Text.Trim()), 20) + "' and SubjectID=" + intSubjectID + "", "LoreName");
				if (strTmp=="")
				{
		...
		private void DataGridLore_UpdateCommand(object source, System.Web.UI.WebControls.DataGridCommandEventArgs e)
		{
            ...
            string strTmp = AccessDateHelper.GetValues("select LoreName from LoreInfo where LoreName='" + ObjFun.getStr(ObjFun.CheckString(strName.Trim()), 20) + "'and LoreID<>" + intLoreID + "", "LoreName");
			if (strTmp=="")
			{
                //string strConn=ConfigurationSettings.AppSettings["strConn"];
                //SqlConnection SqlConn=new SqlConnection(strConn);
                ...
                //SqlConn.Close();
                //SqlConn.Dispose();
                AccessDateHelper.ExecuteNonQuery("update LoreInfo set LoreName='" + ObjFun.getStr(ObjFun.CheckString(strName.Trim()), 20) + "' where LoreID=" + intLoreID);
			}
			else
			{
				this.RegisterStartupScript("newWindow","<script language='javascript'>alert('此知识点名称已经存在！')</script>");
		...
		private void DataGridLore_DeleteCommand(object source, System.Web.UI.WebControls.DataGridCommandEventArgs e)
		{
            ...
            if (AccessDateHelper.GetValues("select RubricID from RubricInfo where LoreID=" + intLoreID + "", "RubricID") == "")
			{
                //string strConn=ConfigurationSettings.AppSettings["strConn"];
                //SqlConnection SqlConn=new SqlConnection(strConn);
                //SqlConn.Open();
                ...
                AccessDateHelper.ExecuteNonQuery("delete from LoreInfo where LoreID=" + intLoreID + "");
		...
		protected void ButDelete_Click(object sender, System.EventArgs e)
		{
			if (Request["chkSelect"]!=null)
			{
				...
				for (int j=0;j<textArray.Length;j++)
				{
                    ...
                    if (AccessDateHelper.GetValues("select RubricID from RubricInfo where LoreID=" + DataGridLore.Items[i].Cells[0].Text.Trim() + "", "RubricID") == "")
					{
                        //SqlCmd=new SqlCommand("delete from LoreInfo where LoreID="+DataGridLore.Items[i].Cells[0].Text.Trim(),SqlConn);
                        //SqlCmd.ExecuteNonQuery();
                        AccessDateHelper.ExecuteNonQuery("delete from LoreInfo where LoreID=" + DataGridLore.Items[i].Cells[0].Text.Trim());
					}
				}
