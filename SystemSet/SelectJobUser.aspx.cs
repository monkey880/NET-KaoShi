...
namespace EasyExam.SystemSet
{
	...
	public partial class SelectJobUser : System.Web.UI.Page
	{
		...
		private void ShowSelectedData()
		{
            ...
            //objDS=new DataSet();
            //objCmd.Fill(objDS,"UserInfo");
            DataSet objDS = AccessDateHelper.ExecuteDataset("select a.UserID,a.LoginID from UserInfo a where a.JobID=" + intJobID + " order by a.LoginID asc");
			for(int i=0;i<objDS.Tables[0].Rows.Count;i++)
			{
		...
		protected void ButInput_Click(object sender, System.EventArgs e)
		{
			...
			try
			{
                ...
                AccessDateHelper.ExecuteNonQuery("Update UserInfo set JobID=0 where JobID=" + intJobID + "");
				...
				for(i=0;i<LBSelected.Items.Count;i++)
				{
                    ...
                    AccessDateHelper.ExecuteNonQuery("Update UserInfo set JobID=" + intJobID + " where UserID=" + LBSelected.Items[i].Value + "");
				}
