...
namespace EasyExam.SystemSet
{
	...
	public partial class ManagSubjectList : System.Web.UI.Page
	{
		...
		private void ShowData(string strSql)
		{
			...
			DataGridSubject.DataSource=SqlDS.Tables[0].DefaultView;
		...
		private void DataGridSubject_UpdateCommand(object source, System.Web.UI.WebControls.DataGridCommandEventArgs e)
		{
            ...
            string strTmp = AccessDateHelper.GetValues("select SubjectName from SubjectInfo where SubjectName='" + ObjFun.getStr(ObjFun.CheckString(strName.Trim()), 20) + "'and SubjectID<>" + intSubjectID + "", "SubjectName");
			if (strTmp=="")
			{
                //string strConn=ConfigurationSettings.AppSettings["strConn"];
                //SqlConnection SqlConn=new SqlConnection(strConn);
                ...
                //SqlConn.Dispose();
                //ShowData(strSql);
                AccessDateHelper.ExecuteNonQuery("update SubjectInfo set SubjectName='" + ObjFun.getStr(ObjFun.CheckString(strName.Trim()), 20) + "' where SubjectID=" + intSubjectID + "");
			}
			else
			{
		...
		private void DataGridSubject_DeleteCommand(object source, System.Web.UI.WebControls.DataGridCommandEventArgs e)
		{
            ...
            if ((AccessDateHelper.GetValues("select LoreID from LoreInfo where SubjectID=" + intSubjectID + "", "LoreID") == "") && (ObjFun.GetValues("select RubricID from RubricInfo where SubjectID=" + intSubjectID + "", "RubricID") == "") && (ObjFun.GetValues("select ChapterID from ChapterInfo where SubjectID=" + intSubjectID + "", "ChapterID") == "") && (ObjFun.GetValues("select SectionID from SectionInfo where SubjectID=" + intSubjectID + "", "SectionID") == ""))
			{
                ...
                AccessDateHelper.ExecuteNonQuery("delete from SubjectInfo where SubjectID=" + intSubjectID + "");
			}
			else
			{
		...
		protected void ButNewDept_Click(object sender, System.EventArgs e)
		{
            ...
            string strTmp = AccessDateHelper.GetValues("select SubjectName from SubjectInfo where SubjectName='" + ObjFun.getStr(ObjFun.CheckString(txtSubjectName.Text.Trim()), 20) + "'", "SubjectName");
			if (strTmp=="")
			{
                //string strConn=ConfigurationSettings.AppSettings["strConn"];
                //SqlConnection SqlConn=new SqlConnection(strConn);
                ...
                //SqlConn.Close();
                //SqlConn.Dispose();
                AccessDateHelper.ExecuteNonQuery("Insert into SubjectInfo(SubjectName,BrowAccount) values('" + ObjFun.getStr(ObjFun.CheckString(txtSubjectName.Text.Trim()), 20) + "',1)");
			...
			if (Request["chkSelect"]!=null)
			{
				...
				for (int j=0;j<textArray.Length;j++)
				{
                    ...
                    if ((AccessDateHelper.GetValues("select LoreID from LoreInfo where SubjectID=" + DataGridSubject.Items[i].Cells[0].Text.Trim() + "", "LoreID") == "") && (ObjFun.GetValues("select RubricID from RubricInfo where SubjectID=" + DataGridSubject.Items[i].Cells[0].Text.Trim() + "", "RubricID") == "") && (ObjFun.GetValues("select ChapterID from ChapterInfo where SubjectID=" + DataGridSubject.Items[i].Cells[0].Text.Trim() + "", "ChapterID") == "") && (ObjFun.GetValues("select SectionID from SectionInfo where SubjectID=" + DataGridSubject.Items[i].Cells[0].Text.Trim() + "", "SectionID") == ""))
					{
                        ...
                        //SqlCmd.ExecuteNonQuery();
                        AccessDateHelper.ExecuteNonQuery("delete from SubjectUser where SubjectID=" + DataGridSubject.Items[i].Cells[0].Text.Trim() + "");
                        AccessDateHelper.ExecuteNonQuery("delete from SubjectInfo where SubjectID=" + DataGridSubject.Items[i].Cells[0].Text.Trim() + "");
					}
				}
