	...
	public partial class SelectMenu : System.Web.UI.Page
	{
		...
		protected void Page_Load(object sender, System.EventArgs e)
		{
			...
			if (!IsPostBack)
			{
                ...
                if (Convert.ToInt32(AccessDateHelper.GetValues("select RoleMenu from UserInfo where UserID=" + intUserID + "", "RoleMenu")) == 1)
				{
...
//				}
		...
		private void ShowSelectedData()
		{
            ...
            DataSet objDS = AccessDateHelper.ExecuteDataset("select b.ManagMenuID,b.MenuName from UserPower a,ManagMenu b where a.OptionID=b.ManagMenuID and a.UserID=" + intUserID + " and a.PowerID=3 order by a.OptionID asc");
...
//			}
			...
			try
			{
                ...
                AccessDateHelper.ExecuteNonQuery("delete from UserPower where UserID=" + intUserID + " and PowerID=3");
				
				if (rbAllMenu.Checked==true)
				{
                    //ObjCmd.CommandText="Update UserInfo set RoleMenu=1 where UserID="+intUserID+"";
                    //ObjCmd.ExecuteNonQuery();
                    AccessDateHelper.ExecuteNonQuery("Update UserInfo set RoleMenu=1 where UserID=" + intUserID + "");
				}
				else
				{
					for(i=0;i<LBSelected.Items.Count;i++)
					{
                        ...
                        //ObjCmd.ExecuteNonQuery();
                        AccessDateHelper.ExecuteNonQuery("insert into UserPower(UserID,PowerID,OptionID) values(" + intUserID + ",3," + LBSelected.Items[i].Value + ")");
					}
                    //ObjCmd.CommandText="Update UserInfo set RoleMenu=2 where UserID="+intUserID+"";
                    //ObjCmd.ExecuteNonQuery();
                    AccessDateHelper.ExecuteNonQuery("Update UserInfo set RoleMenu=2 where UserID=" + intUserID + "");
				}
