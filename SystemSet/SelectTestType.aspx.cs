...
namespace EasyExam.SystemSet
{
	...
	public partial class SelectTestType : System.Web.UI.Page
	{
		...
		protected void Page_Load(object sender, System.EventArgs e)
		{
			...
			if (!IsPostBack)
			{
                if (Convert.ToInt32(AccessDateHelper.GetValues("select JudgeTestType from UserInfo where UserID=" + intUserID + "", "JudgeTestType")) == 1)
				{
...
//				}
		...
		private void ShowData(string strSql)
		{
			...
			LBSelect.DataSource=objDS.Tables[0].DefaultView;
			LBSelect.DataBind();
			//objCmd.Dispose();
			objDS.Dispose();
			//objConn.Dispose();
		...
		private void ShowSelectedData()
		{
            ...
            DataSet objDS = AccessDateHelper.ExecuteDataset("select b.TestTypeID,b.TestTypeName from UserPower a,TestTypeInfo b where a.OptionID=b.TestTypeID and a.UserID=" + intUserID + " and a.PowerID=2 order by a.OptionID asc");
			LBSelected.DataTextField="TestTypeName";
			LBSelected.DataValueField="TestTypeID";
			LBSelected.DataSource=objDS.Tables[0].DefaultView;
			LBSelected.DataBind();
			//objCmd.Dispose();
			objDS.Dispose();
			//objConn.Dispose();
...
//			}
			...
			try
			{
                ...
                AccessDateHelper.ExecuteNonQuery("delete from UserPower where UserID=" + intUserID + " and PowerID=2");
				...
				if (rbAllTestType.Checked==true)
				{
                    ...
                    AccessDateHelper.ExecuteNonQuery("Update UserInfo set JudgeTestType=1 where UserID=" + intUserID + "");
				}
				else
				{
					for(i=0;i<LBSelected.Items.Count;i++)
					{
                        ...
                        //ObjCmd.ExecuteNonQuery();
                        AccessDateHelper.ExecuteNonQuery("insert into UserPower(UserID,PowerID,OptionID) values(" + intUserID + ",2," + LBSelected.Items[i].Value + ")");
					}
                    //ObjCmd.CommandText="Update UserInfo set JudgeTestType=2 where UserID="+intUserID+"";
                    //ObjCmd.ExecuteNonQuery();
                    AccessDateHelper.ExecuteNonQuery("Update UserInfo set JudgeTestType=2 where UserID=" + intUserID + "");
				}
