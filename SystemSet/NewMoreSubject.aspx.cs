	...
	public partial class NewMoreSubject : System.Web.UI.Page
	{
		...
		protected void ButInput_Click(object sender, System.EventArgs e)
		{
			...
			for(long i=0;i<strArrSubject.Length;i++)
			{
				...
				{
                    string strTmp = AccessDateHelper.GetValues("select SubjectName from SubjectInfo where SubjectName='" + ObjFun.getStr(ObjFun.CheckString(strArrSubject[i].Trim()), 20) + "'", "SubjectName");
					if (strTmp.Trim()!="")
					{
			...
			try
			{
				for(long i=0;i<strArrSubject.Length;i++)
				{
					if(strArrSubject[i].Trim()!="")
					{
                        ...
                        AccessDateHelper.ExecuteNonQuery("insert into SubjectInfo(SubjectName) values('" + ObjFun.getStr(ObjFun.CheckString(strArrSubject[i].Trim()), 20) + "')");
					}
				}
				//ObjTran.Commit();
