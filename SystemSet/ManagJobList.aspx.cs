...
namespace EasyExam.SystemSet
{
	...
	public partial class ManagJobList : System.Web.UI.Page
	{
		...
		protected void ButNewDept_Click(object sender, System.EventArgs e)
		{
            ...
            string strTmp = AccessDateHelper.GetValues("select JobName from JobInfo where JobName='" + ObjFun.getStr(ObjFun.CheckString(txtJobName.Text.Trim()), 20) + "'", "JobName");
			if (strTmp=="")
			{
                ...
                //SqlConn.Close();
                //SqlConn.Dispose();
                AccessDateHelper.ExecuteNonQuery("Insert into JobInfo(JobName) values('" + ObjFun.getStr(ObjFun.CheckString(txtJobName.Text.Trim()), 20) + "')");
		...
		private void DataGridJob_DeleteCommand(object source, System.Web.UI.WebControls.DataGridCommandEventArgs e)
		{
            ...
            if (AccessDateHelper.GetValues("select UserID from UserInfo where JobID=" + intJobID + "", "UserID") == "")
			{
                //string strConn=ConfigurationSettings.AppSettings["strConn"];
                //SqlConnection SqlConn=new SqlConnection(strConn);
                //SqlConn.Open();
                ...
                AccessDateHelper.ExecuteNonQuery("delete from JobInfo where JobID=" + intJobID + "");
		...
		private void DataGridJob_UpdateCommand(object source, System.Web.UI.WebControls.DataGridCommandEventArgs e)
		{
            ...
            string strTmp = AccessDateHelper.GetValues("select JobName from JobInfo where JobName='" + ObjFun.getStr(ObjFun.CheckString(strName.Trim()), 20) + "'and JobID<>" + intJobID + "", "JobName");
			if (strTmp=="")
			{
                //string strConn=ConfigurationSettings.AppSettings["strConn"];
                //SqlConnection SqlConn=new SqlConnection(strConn);
                ...
                //SqlConn.Close();
                //SqlConn.Dispose();
                AccessDateHelper.ExecuteNonQuery("update JobInfo set JobName='" + ObjFun.getStr(ObjFun.CheckString(strName.Trim()), 20) + "' where JobID=" + intJobID + "");
			}
			else
			{
				this.RegisterStartupScript("newWindow","<script language='javascript'>alert('此职务名称已经存在！')</script>");
		...
		private void ShowData(string strSql)
		{
			...
			DataGridJob.DataSource=SqlDS.Tables[0].DefaultView;
		...
		protected void ButDelete_Click(object sender, System.EventArgs e)
		{
			if (Request["chkSelect"]!=null)
			{
				...
				for (int j=0;j<textArray.Length;j++)
				{
                    ...
                    if (AccessDateHelper.GetValues("select UserID from UserInfo where JobID='" + DataGridJob.Items[i].Cells[0].Text.Trim() + "'", "UserID") == "")
					{
                        //SqlCmd=new SqlCommand("delete from JobInfo where JobID="+DataGridJob.Items[i].Cells[0].Text.Trim(),SqlConn);
                        //SqlCmd.ExecuteNonQuery();
                        AccessDateHelper.ExecuteNonQuery("delete from JobInfo where JobID=" + DataGridJob.Items[i].Cells[0].Text.Trim());
					}
				}
