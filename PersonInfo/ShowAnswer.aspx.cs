		...
		protected void Page_Load(object sender, System.EventArgs e)
		{
			...
			//{
				...
				if (intUserScoreID!=0)
				{
                    ...
                    SqlDS = AccessDateHelper.ExecuteDataset("select a.*,b.LoginID,b.UserName from UserScore a,UserInfo b where a.UserScoreID=" + intUserScoreID + " and b.UserID=a.UserID order by a.UserScoreID asc");

					//SqlCmd.Fill(SqlDS,"UserScore");
					strLoginID=SqlDS.Tables[0].Rows[0]["LoginID"].ToString();
					strUserName=SqlDS.Tables[0].Rows[0]["UserName"].ToString();
					intPaperID=Convert.ToInt32(SqlDS.Tables[0].Rows[0]["PaperID"]);
					strExamTime=SqlDS.Tables[0].Rows[0]["StartTime"].ToString()+"/"+SqlDS.Tables[0].Rows[0]["EndTime"].ToString();
					strTotalMark=SqlDS.Tables[0].Rows[0]["TotalMark"].ToString();
                    ...
                    SqlDSTestType = AccessDateHelper.ExecuteDataset("select a.TestTypeID,b.BaseTestType,a.TestTypeTitle,a.TestTypeMark,a.TestAmount,a.TestTotalMark from PaperTestType a,TestTypeInfo b where a.TestTypeID=b.TestTypeID and a.PaperID=" + intPaperID + " order by a.PaperTestTypeID asc");
					...
					for(int i=0;i<SqlDSTestType.Tables[0].Rows.Count;i++)
					{
						...
						{
							dblTotalMark=System.Math.Round(Convert.ToDouble(SqlDSTestType.Tables[0].Rows[0]["TestTotalMark"].ToString()),2);
						}
						strPaperContent=strPaperContent+"<tr>";
						strPaperContent=strPaperContent+"<td colspan='2' bgcolor='#CEE7FF' height='20' style='CURSOR:hand' onclick='jscomFlexObject(trTestTypeContent"+Convert.ToString(i+1)+")' title='点击伸缩该大题'>"+ObjFun.convertint(Convert.ToString(i+1))+"."+SqlDSTestType.Tables[0].Rows[i]["TestTypeTitle"].ToString()+"（共"+SqlDSTestType.Tables[0].Rows[i]["TestAmount"].ToString()+"题,共"+dblTotalMark.ToString()+"分）</td>";
						strPaperContent=strPaperContent+"</tr>";
						strPaperContent=strPaperContent+"<tr>";
						strPaperContent=strPaperContent+"<td width='10' nowrap></td>";
						strPaperContent=strPaperContent+"<td width='100%'>";
						strPaperContent=strPaperContent+"<table cellSpacing='0' cellPadding='1' width='100%' align='center' border='0' id='trTestTypeContent"+Convert.ToString(i+1)+"'><tr><td>";

                        //SqlCmdTest=new SqlDataAdapter("select a.TestOrder,a.RubricID,b.TestTypeID,b.TestDiff,b.OptionNum,a.TestMark,b.TestContent,a.TestFile,a.TestFileName,b.OptionContent,b.StandardAnswer,b.TestParse,a.UserAnswer,a.UserScore from UserAnswer a,RubricInfo b where a.RubricID=b.RubricID and b.TestTypeID="+SqlDSTestType.Tables[0].Rows[i]["TestTypeID"]+" and a.UserScoreID="+intUserScoreID+" order by a.TestOrder asc",SqlConn);
                        //SqlDSTest=new DataSet();
                        //SqlCmdTest.Fill(SqlDSTest,"UserAnswer");

                        SqlDSTest = AccessDateHelper.ExecuteDataset("select a.TestOrder,a.RubricID,b.TestTypeID,b.TestDiff,b.OptionNum,a.TestMark,b.TestContent,a.TestFile,a.TestFileName,b.OptionContent,b.StandardAnswer,b.TestParse,a.UserAnswer,a.UserScore from UserAnswer a,RubricInfo b where a.RubricID=b.RubricID and b.TestTypeID=" + SqlDSTestType.Tables[0].Rows[i]["TestTypeID"] + " and a.UserScoreID=" + intUserScoreID + " order by a.TestOrder asc");
						...
						for(int j=0;j<SqlDSTest.Tables[0].Rows.Count;j++)
						{
							...
							if ((RadioButAll.Checked)||((RadioButWrong.Checked)&&(Convert.ToDouble(SqlDSTest.Tables[0].Rows[j]["UserScore"].ToString())==0)))
							{
								strPaperContent=strPaperContent+"<tr>";
								if (SqlDSTestType.Tables[0].Rows[i]["BaseTestType"].ToString()=="填空类")
								{
									...
									strTestContent=SqlDSTest.Tables[0].Rows[j]["TestContent"].ToString();
									strArrOptionContent=SqlDSTest.Tables[0].Rows[j]["UserAnswer"].ToString().Split(',');
								...
								if (SqlDSTestType.Tables[0].Rows[i]["BaseTestType"].ToString()=="单选类")
								{
									...
									{
										strPaperContent=strPaperContent+"<tr>";
										if (SqlDSTest.Tables[0].Rows[j]["UserAnswer"].ToString().IndexOf(Convert.ToChar(64+k))>=0)
										{
								...
								if (SqlDSTestType.Tables[0].Rows[i]["BaseTestType"].ToString()=="判断类")
								{
									...
									if (SqlDSTest.Tables[0].Rows[j]["UserAnswer"].ToString().IndexOf("正确")>=0)
									{
									...
									strPaperContent=strPaperContent+"<tr>";
									strPaperContent=strPaperContent+"<td width='10' nowrap>";
									if (SqlDSTest.Tables[0].Rows[j]["UserAnswer"].ToString().IndexOf("错误")>=0)
									{
								...
								{
									strPaperContent=strPaperContent+"<tr>";
									strPaperContent=strPaperContent+"<td width='10px' nowrap><td width='100%'><TEXTAREA rows=6 cols=40 id='Answer"+intTestNum.ToString()+"' name='Answer"+intTestNum.ToString()+"' style='width:100%' class=Text readonly>"+SqlDSTest.Tables[0].Rows[j]["UserAnswer"].ToString()+"</TEXTAREA></td>";
									strPaperContent=strPaperContent+"</tr>";
								}
								if (SqlDSTestType.Tables[0].Rows[i]["BaseTestType"].ToString()=="作文类")
								{
									strPaperContent=strPaperContent+"<tr>";
									strPaperContent=strPaperContent+"<td width='10px' nowrap><td width='100%'><TEXTAREA rows=10 cols=40 id='Answer"+intTestNum.ToString()+"' name='Answer"+intTestNum.ToString()+"' style='width:100%' class=Text readonly>"+SqlDSTest.Tables[0].Rows[j]["UserAnswer"].ToString()+"</TEXTAREA></td>";
									strPaperContent=strPaperContent+"</tr>";
								}
								...
								{
									strPaperContent=strPaperContent+"<tr>";
									strPaperContent=strPaperContent+"<td width='10px' nowrap><td width='100%'><a href='../PersonInfo/DownLoadFile.aspx?UserScoreID="+intUserScoreID+"&RubricID="+SqlDSTest.Tables[0].Rows[j]["RubricID"].ToString()+"' title='点击下载'>下载文件：</a><b>"+SqlDSTest.Tables[0].Rows[j]["TestFileName"].ToString()+"</b></td>";
									strPaperContent=strPaperContent+"</tr>";
								}
								if (SqlDSTestType.Tables[0].Rows[i]["BaseTestType"].ToString()=="打字类")
								{
									...
									if (SqlDSTest.Tables[0].Rows[j]["UserAnswer"].ToString().IndexOf(",")>=0)
									{
										strArrTypeUserAnswer=SqlDSTest.Tables[0].Rows[j]["UserAnswer"].ToString().Split(',');
									}
									else
									{
								...
								{
									strPaperContent=strPaperContent+"<tr>";
									strPaperContent=strPaperContent+"<td colspan='2'><font color='blue'>标准答案："+SqlDSTest.Tables[0].Rows[j]["StandardAnswer"].ToString()+"</font></td>";
									strPaperContent=strPaperContent+"</tr>";

									strPaperContent=strPaperContent+"<tr>";
									strPaperContent=strPaperContent+"<td colspan='2'><font color='blue'>考生答案："+SqlDSTest.Tables[0].Rows[j]["UserAnswer"].ToString()+"</font></td>";
									strPaperContent=strPaperContent+"</tr>";
								}

								strPaperContent=strPaperContent+"<tr>";
								if (Convert.ToDouble(SqlDSTest.Tables[0].Rows[j]["UserScore"])==0)
								{
									strPaperContent=strPaperContent+"<td colspan='2'><font color='red'>本题得分："+SqlDSTest.Tables[0].Rows[j]["UserScore"].ToString()+"</font></td>";
								}
								else
								{
									strPaperContent=strPaperContent+"<td colspan='2'><font color='blue'>本题得分："+SqlDSTest.Tables[0].Rows[j]["UserScore"].ToString()+"</font></td>";
								}
								strPaperContent=strPaperContent+"</tr>";
