		...
		protected void Page_Load(object sender, System.EventArgs e)
		{
			...
			if (!IsPostBack)
			{
				if (intPaperID!=0)
				{
                    ...
                    intUserScoreID = Convert.ToInt32(AccessDateHelper.GetValues("select UserScoreID from UserScore where PaperID=" + intPaperID + " and UserID=" + intUserID + "", "UserScoreID"));
                    intRemTime = Convert.ToInt32(AccessDateHelper.GetValues("select RemTime from UserScore where PaperID=" + intPaperID + " and UserID=" + intUserID + "", "RemTime"));
                    ...
                    DataSet SqlDS = AccessDateHelper.ExecuteDataset("select PaperName,ProduceWay,TestCount,PaperMark,ExamTime,AutoSave,PassMark,FillAutoGrade,SeeResult,AutoJudge from PaperInfo where PaperID=" + intPaperID + " order by PaperID asc");
					...
					else
					{
                        ...
                        AccessDateHelper.ExecuteNonQuery("Update UserAnswer set UserAnswer='" + ObjFun.getStr(strUserAnswer, 2000) + "',UserScore=" + dblUserScore + " where UserScoreID=" + intUserScoreID + " ");
                        ...
                        AccessDateHelper.ExecuteNonQuery("Update UserScore set EndTime=date(),RemTime=" + intRemTime + " where UserScoreID=" + intUserScoreID + "");
                    ...
                    DataSet SqlDSTest = AccessDateHelper.ExecuteDataset("select a.PaperTestTypeID,a.TestTypeID,b.BaseTestType,a.TestTypeTitle,a.TestAmount,a.TestTotalMark,a.TestTypeOrder,c.TestOrder,c.RubricID,d.OptionNum,c.TestMark,d.TestContent,c.TestFileName,d.OptionContent,c.UserAnswer from PaperTestType a,TestTypeInfo b,UserAnswer c,RubricInfo d where a.TestTypeID=b.TestTypeID and c.RubricID=d.RubricID and d.TestTypeID=a.TestTypeID and c.UserScoreID=" + intUserScoreID + " and a.PaperID=" + intPaperID + " and c.TestOrder=" + intTestNum.ToString() + " order by a.PaperTestTypeID asc,c.TestOrder asc");

					

					if (intProduceWay==3)//试题随机
					{
                        dblTotalMark = System.Math.Round(Convert.ToDouble(AccessDateHelper.GetValues("select sum(a.TestMark) as TotalMark from UserAnswer a,RubricInfo b where a.RubricID=b.RubricID and b.TestTypeID=" + SqlDSTest.Tables[0].Rows[0]["TestTypeID"] + " and a.UserScoreID=" + intUserScoreID + "", "TotalMark")), 2);
					}
					else//试题固定、题序随机
					{
						dblTotalMark=System.Math.Round(Convert.ToDouble(SqlDSTest.Tables[0].Rows[0]["TestTotalMark"].ToString()),2);
					}
					strTestTypeMark=ObjFun.convertint(SqlDSTest.Tables[0].Rows[0]["TestTypeOrder"].ToString())+"."+SqlDSTest.Tables[0].Rows[0]["TestTypeTitle"].ToString()+"（共"+SqlDSTest.Tables[0].Rows[0]["TestAmount"].ToString()+"题,共"+dblTotalMark.ToString()+"分）";
					...
					if (SqlDSTest.Tables[0].Rows[0]["BaseTestType"].ToString()=="单选类")
					{
						...
						{
							strPaperContent=strPaperContent+"<tr>";
							if (SqlDSTest.Tables[0].Rows[0]["UserAnswer"].ToString().IndexOf(Convert.ToChar(64+k))>=0)
							{
					...
					if (SqlDSTest.Tables[0].Rows[0]["BaseTestType"].ToString()=="判断类")
					{
						...
						if (SqlDSTest.Tables[0].Rows[0]["UserAnswer"].ToString().IndexOf("正确")>=0)
						{
						...
						strPaperContent=strPaperContent+"<tr>";
						strPaperContent=strPaperContent+"<td width='10' nowrap>";
						if (SqlDSTest.Tables[0].Rows[0]["UserAnswer"].ToString().IndexOf("错误")>=0)
						{
					...
					{
						strPaperContent=strPaperContent+"<tr>";
						strPaperContent=strPaperContent+"<td width='10px' nowrap><td width='100%'><TEXTAREA rows=6 cols=40 id='Answer"+intTestNum.ToString()+"' name='Answer"+intTestNum.ToString()+"' style='width:100%' class=Text>"+SqlDSTest.Tables[0].Rows[0]["UserAnswer"].ToString()+"</TEXTAREA></td>";
						strPaperContent=strPaperContent+"</tr>";
					}
					if (SqlDSTest.Tables[0].Rows[0]["BaseTestType"].ToString()=="作文类")
					{
						strPaperContent=strPaperContent+"<tr>";
						strPaperContent=strPaperContent+"<td width='10px' nowrap><td width='100%'><TEXTAREA rows=10 cols=40 id='Answer"+intTestNum.ToString()+"' name='Answer"+intTestNum.ToString()+"' style='width:100%' class=Text>"+SqlDSTest.Tables[0].Rows[0]["UserAnswer"].ToString()+"</TEXTAREA></td>";
						strPaperContent=strPaperContent+"</tr>";
					}
					if (SqlDSTest.Tables[0].Rows[0]["BaseTestType"].ToString()=="打字类")
					{
						if (SqlDSTest.Tables[0].Rows[0]["UserAnswer"].ToString().IndexOf(",")>=0)
						{
							strArrTypeUserAnswer=SqlDSTest.Tables[0].Rows[0]["UserAnswer"].ToString().Split(',');
						}
						else
						{
					...
					if (Request["TrySubmit"]!=null)//尝试提交
					{
                        ...
                        //SqlCmdTest.Fill(SqlDSTest,"UserAnswer");
                         ...
                         SqlDSTest = AccessDateHelper.ExecuteDataset("select a.PaperTestTypeID,b.BaseTestType,a.TestTypeTitle,c.TestOrder,c.UserAnswer from PaperTestType a,TestTypeInfo b,UserAnswer c,RubricInfo d where a.TestTypeID=b.TestTypeID and c.RubricID=d.RubricID and d.TestTypeID=a.TestTypeID and c.UserScoreID=" + intUserScoreID + " and a.PaperID=" + intPaperID + " order by a.PaperTestTypeID,c.TestOrder asc");

						int intNotFinished=0;
						string strTestTypeTitle="";
						for(i=0;i<SqlDSTest.Tables[0].Rows.Count;i++)
						{
							strTestTypeTitle=SqlDSTest.Tables[0].Rows[i]["TestTypeTitle"].ToString();
