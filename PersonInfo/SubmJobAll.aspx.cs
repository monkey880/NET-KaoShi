		...
		protected void Page_Load(object sender, System.EventArgs e)
		{
			...
			if (intPaperID!=0)
			{
                ...
                int type_dx4 = Convert.ToInt16(AccessDateHelper.ExecuteScalar("select count(r.RubricID) from PaperTest p left join RubricInfo r on p.RubricID=r.RubricID where r.TestTypeID=30 and p.PaperID=" + intPaperID + ""));
					...
					strMessage+="<table width='100%' border='0' cellpadding='10' cellspacing='1' bgcolor='#52D6E9'><tr><td height='51' colspan='2' align='center' valign='middle' bgcolor='#CDF1FC' style='font-size:24px; font-family:'Microsoft YaHei UI', Arial'>作业情况统计</td></tr><tr>";
    strMessage+="<td width='46%' bgcolor='#FFFFFF'>试题总数："+testnum+"题<br />";
    strMessage+="单选题："+type_dx+"题<br />";
    strMessage += " 多选题：" + type_dx2 + "题<br />";
    strMessage += " 真空题：" + type_dx4 + "题<br />";
    strMessage += "判断题：" + type_dx3 + "题</td>";
    strMessage += "<td width='54%' bgcolor='#FFFFFF'>您做对：" + cornum + "题</td>";
  strMessage+="</tr>";
  strMessage+="<tr>";
    strMessage+="<td bgcolor='#FFFFFF'>参加人数："+testrennum+"</td>";
    strMessage += "<td bgcolor='#FFFFFF'>您做错：" + (testnum-cornum) + "题</td>";
  strMessage+="</tr>";
  strMessage += "<tr>";
  strMessage = strMessage + "<td align'right'><input class='button' type='button' name='cancel'  value='查看答卷' onclick=javascript:{NewWin=window.open('ShowMyAnswer.aspx?PaperID=" + intPaperID + "','ShowAnswer','titlebar=yes,menubar=no,toolbar=no,location=no,directories=no,status=yes,scrollbars=yes,resizable=no,copyhistory=yes,top=0,left=0,width=screen.availWidth,height=screen.availHeight');NewWin.moveTo(0,0);NewWin.resizeTo(screen.availWidth,screen.availHeight);}></td>";
