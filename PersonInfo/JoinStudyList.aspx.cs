	...
	public partial class JoinStudyList : System.Web.UI.Page
	{
		...
		protected void Page_Load(object sender, System.EventArgs e)
		{
			...
			if (!IsPostBack)
			{
				...
				bWhere=true;
                strSql = "select a.SectionID,c.SubjectName,b.ChapterName,a.SectionName,a.BrowNumber,d.LoginID as CreateLoginID from ((SectionInfo a LEFT JOIN ChapterInfo b ON a.ChapterID = b.ChapterID) LEFT JOIN SubjectInfo c ON a.SubjectID = c.SubjectID) LEFT JOIN UserInfo d ON b.CreateUserID = d.UserID where (c.BrowAccount=1 or (c.BrowAccount=2 and Exists(select * from SubjectUser d where d.SubjectID=c.SubjectID and d.UserID=" + intUserID + ")) or Exists(select * from UserInfo e,DeptInfo f,SubjectUser g where e.UserID=" + intUserID + " and e.DeptID=f.DeptID and f.DeptID=g.DeptID and g.SubjectID=c.SubjectID))";
				//显示以科目为条件的数据
				if (intSubjectID!=0)
				{
					if (bWhere)
		...
		private void ShowData(string strSql)
		{
			...
			DataGridBook.DataSource=SqlDS.Tables[0].DefaultView;
