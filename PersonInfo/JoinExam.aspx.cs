...
namespace EasyExam.PersonalInfo
{
	...
	public partial class JoinExam : System.Web.UI.Page
	{
		...
		protected void Page_Load(object sender, System.EventArgs e)
		{
            ...
            //strSql = "select a.PaperID,a.PaperName,a.PaperType,iif( a.ProduceWay = 1, '题序固定',iif(a.ProductWay= 2,'题序随机',iif(a.ProductWay= 3 ,'试题随机'))) as ProduceWay,a.ShowModal,a.ExamTime,a.StartTime,a.EndTime,a.TestCount,a.PaperMark,a.CreateWay,b.LoginID as CreateLoginID from PaperInfo a LEFT  JOIN UserInfo b ON a.CreateUserID=b.UserID where a.PaperType=1 and a.StartTime<Date() and a.EndTime>Date() and (a.ExamAccount=1 or (a.ExamAccount=2 and Exists(select * from PaperUser where PaperUser.PaperID=a.PaperID and PaperUser.UserType=0 and PaperUser.UserID=" + intUserID + ")) or Exists(select * from UserInfo d,DeptInfo e,PaperUser f where d.UserID=" + intUserID + " and d.DeptID=e.DeptID and e.DeptID=f.DeptID and f.PaperID=a.PaperID and f.UserType=0)) and (a.RepeatExam=1 or ((a.RepeatExam=0) and (not Exists (select * from UserScore where UserScore.PaperID=a.PaperID and UserScore.UserID=" + intUserID + " and UserScore.ExamState=1)))) order by a.PaperID desc";

            strSql = "select a.PaperID,a.PaperName,a.PaperType,a.ProduceWay as ProduceWay,a.ShowModal,a.ExamTime,a.StartTime,a.EndTime,a.TestCount,a.PaperMark,a.CreateWay,b.LoginID as CreateLoginID from PaperInfo a LEFT  JOIN UserInfo b ON a.CreateUserID=b.UserID where a.PaperType=1 and a.StartTime<Date() and a.EndTime>Date() and (a.ExamAccount=1 or (a.ExamAccount=2 and Exists(select * from PaperUser where PaperUser.PaperID=a.PaperID and PaperUser.UserType=0 and PaperUser.UserID=" + intUserID + ")) or Exists(select * from UserInfo d,DeptInfo e,PaperUser f where d.UserID=" + intUserID + " and d.DeptID=e.DeptID and e.DeptID=f.DeptID and f.PaperID=a.PaperID and f.UserType=0)) and (a.RepeatExam=1 or ((a.RepeatExam=0) and (not Exists (select * from UserScore where UserScore.PaperID=a.PaperID and UserScore.UserID=" + intUserID + " and UserScore.ExamState=1)))) order by a.PaperID desc";
			if (!IsPostBack)
			{
		...
		private void ShowData(string strSql)
		{
			...
			{				
				...
				HyperLink LBStartExam=(HyperLink)DataGridPaper.Items[i].FindControl("LinkButStartExam");
				if (DataGridPaper.Items[i].Cells[14].Text.Trim()=="1")
				{
                    //LBStartExam.Attributes.Add("onclick","javascript:{if (confirm('您确定要开始考试吗？')==true) {NewExam=window.open('StartExamAll.aspx?PaperID="+DataGridPaper.Items[i].Cells[0].Text.Trim()+"&UserID="+intUserID+"&Start=yes','StartExamAll','titlebar=yes,menubar=no,toolbar=no,location=no,directories=no,status=yes,scrollbars=yes,resizable=no,copyhistory=yes,top=0,left=0,width=screen.availWidth,height=screen.availHeight');NewExam.moveTo(0,0);NewExam.resizeTo(screen.availWidth,screen.availHeight);}else{return false;}}");
                    LBStartExam.NavigateUrl = "StartExamAll.aspx?PaperID=" + DataGridPaper.Items[i].Cells[0].Text.Trim() + "&UserID=" + intUserID + "&Start=yes";
                    LBStartExam.Target = "_blank";

				}
				if (DataGridPaper.Items[i].Cells[14].Text.Trim()=="2")
				{
                    //LBStartExam.Attributes.Add("onclick","javascript:{if (confirm('您确定要开始考试吗？')==true) {NewExam=window.open('StartExamOne.aspx?PaperID="+DataGridPaper.Items[i].Cells[0].Text.Trim()+"&UserID="+intUserID+"&Start=yes','StartExamOne','titlebar=yes,menubar=no,toolbar=no,location=no,directories=no,status=yes,scrollbars=yes,resizable=no,copyhistory=yes,top=0,left=0,width=screen.availWidth,height=screen.availHeight');NewExam.moveTo(0,0);NewExam.resizeTo(screen.availWidth,screen.availHeight);}else{return false;}}");
                    LBStartExam.NavigateUrl = "StartExamOne.aspx?PaperID=" + DataGridPaper.Items[i].Cells[0].Text.Trim() + "&UserID=" + intUserID + "&Start=yes";
                    LBStartExam.Target = "_blank";
				}
			}
