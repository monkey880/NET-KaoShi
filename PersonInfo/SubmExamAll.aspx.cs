		...
		protected void Page_Load(object sender, System.EventArgs e)
		{
			...
			if ((intPaperID!=0)&&(intUserScoreID!=0))
			{
				try
				{
                    ...
                    SqlDSTest = AccessDateHelper.ExecuteDataset("select a.TestMark,b.StandardAnswer from UserAnswer a,RubricInfo b,PaperTestType c where a.RubricID=b.RubricID and b.TestTypeID=c.TestTypeID and a.UserScoreID=" + intUserScoreID + " and c.PaperID=" + intPaperID + " order by c.PaperTestTypeID,a.TestOrder");
					...
					for(int i=0;i<SqlDSTest.Tables[0].Rows.Count;i++)
					{
							...
							case "填空类":
								...
								if (intFillAutoGrade==1)
								{
									...
									for(int j=1;j<=Math.Min(strArrUserAnswer.Length,strArrStandardAnswer.Length);j++)
									{
										...
										{
											dblUserScore=dblUserScore+Convert.ToDouble(SqlDSTest.Tables[0].Rows[i]["TestMark"])*(1/Convert.ToDouble(Math.Min(strArrUserAnswer.Length,strArrStandardAnswer.Length)));
										}
									}
						...
						//保存答案、得分
						//ObjCmd.CommandText="Update UserAnswer set UserAnswer='"+ObjFun.getStr(strUserAnswer,2000)+"',UserScore="+dblUserScore+" where UserScoreID="+intUserScoreID+" and TestOrder="+SqlDSTest.Tables[0].Rows[i]["TestOrder"]+"";
						//ObjCmd.ExecuteNonQuery();

                        AccessDateHelper.ExecuteNonQuery("Update UserAnswer set UserAnswer='" + ObjFun.getStr(strUserAnswer, 2000) + "',UserScore=" + dblUserScore + " where UserScoreID=" + intUserScoreID + " and TestOrder=" + SqlDSTest.Tables[0].Rows[i]["TestOrder"] + "");
                    ...
                    AccessDateHelper.ExecuteNonQuery("Update UserScore set ExamState=1,EndTime=date(),RemTime=" + intRemTime + ",LoginIP='" + Convert.ToString(Request.ServerVariables["Remote_Addr"]) + "',JudgeState=" + intJudgeState + ",JudgeUserID=" + intJudgeUserID + ",ImpScore=" + dblImpScore + ",SubScore=" + dblSubScore + ",PassState=" + intPassState + ",TotalMark=" + (dblImpScore + dblSubScore) + " where UserScoreID=" + intUserScoreID + "");
