	...
	public partial class QueryGrade : System.Web.UI.Page
	{
		...
		protected void Page_Load(object sender, System.EventArgs e)
		{
			...
			if (!IsPostBack)
			{
                if (intPaperType > 0)
                {
                     wheresql = " and b.PaperType=" + intPaperType ;
                }
                strSql = "select a.UserScoreID,a.PaperID,b.PaperName,b.SeeResult,a.ExamState,a.StartTime,a.EndTime,switch( a.JudgeState = 0 , '未评' ,a.JudgeState = 1 , '已评' ) as JudgeState,c.LoginID as JudgeLoginID,switch( a.PassState = 0 , '未通过' ,a.PassState = 1 , '通过' ) as PassState,a.TotalMark from PaperInfo b RIGHT JOIN (UserInfo c RIGHT JOIN UserScore a ON c.UserID = a.UserID) ON b.PaperID = a.PaperID where a.ExamState=1 " + wheresql + " and b.SeeResult=1 and a.UserID=" + intUserID + " order by UserScoreID desc";
		...
		private void ShowData(string strSql)
		{
			...
			DataGridGrade.DataSource=SqlDS.Tables[0].DefaultView;
			...
			for(int i=0;i<DataGridGrade.Items.Count;i++)
			{
				...
				else
				{
					...
					//	}
					//}
                    intOrder = Convert.ToInt32(AccessDateHelper.GetValues("select Count(*) as countnum from UserScore where PaperID=" + DataGridGrade.Items[i].Cells[1].Text.Trim() + " and ExamState=1 and TotalMark >" + labTotalMark.Text + "", "countnum")) + 1;
		...
		{
			bWhere=true;
            strSql = "select a.UserScoreID,a.PaperID,b.PaperName,b.SeeResult,a.ExamState,a.StartTime,a.EndTime,switch( a.JudgeState = 0 , '未评' ,a.JudgeState = 1 , '已评' ) as JudgeState,c.LoginID as JudgeLoginID,switch( a.PassState = 0 , '未通过' ,a.PassState = 1 , '通过' ) as PassState,a.TotalMark from (UserScore a LEFT  JOIN PaperInfo b ON a.PaperID=b.PaperID ) LEFT  JOIN UserInfo c ON a.JudgeUserID=c.UserID where a.ExamState=1 and b.PaperType=" + intPaperType + " and b.SeeResult=1 and a.UserID=" + intUserID + "";
			//显示以名称为条件的数据
