        ...
        protected void Page_Load(object sender, System.EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                ...
                else
                {
                    ...
                    {
                        ButAddJobPaper.PostBackUrl = "AddJobPaper.aspx";
                        ButDelete.Attributes.Add("onclick", "javascript:{if(confirm('确定要删除选择试卷吗？')==false) return false;}");
                        strSql = "select a.PaperID,a.PaperName,a.PaperType,SWITCH(  a.ProduceWay = 1 , '题序固定' , a.ProduceWay = 2 , '题序随机' ,a.ProduceWay = 3 , '试题随机' ) as ProduceWay,SWITCH( a.ShowModal = 1 , '整卷模式' ,a.ShowModal = 2 , '逐题模式' ) as ShowModal,a.StartTime,a.EndTime,a.TestCount,a.PaperMark,a.CreateWay,b.LoginID as CreateLoginID,a.CreateDate as CreateDate from PaperInfo a LEFT  JOIN UserInfo b ON a.CreateUserID=b.UserID where a.PaperType=2 order by a.PaperID desc";
		...
		private void ShowData(string strSql)
		{
			...
			{				
                ...
                //{
					...
					{
						LBEditPaper.Attributes.Add("onclick", "jscomNewOpenBySize('EditRandPaper.aspx?PaperID="+DataGridPaper.Items[i].Cells[0].Text.Trim()+"&PaperType="+DataGridPaper.Items[i].Cells[8].Text.Trim()+"','EditRandPaper',700,600);return false;");
					}
					else
					{
						LBEditPaper.Attributes.Add("onclick", "jscomNewOpenBySize('EditCustomPaper.aspx?PaperID="+DataGridPaper.Items[i].Cells[0].Text.Trim()+"&PaperType="+DataGridPaper.Items[i].Cells[8].Text.Trim()+"','EditCustomPaper',700,600);return false;");
					}
