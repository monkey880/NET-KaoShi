...
namespace EasyExam.Teacher
{
	...
	public partial class ManagUserList : System.Web.UI.Page
	{
		...
		{			
			...
			if (!IsPostBack)
			{
				...
				else
				{
                    ...
                    strSql = "select UserID,LoginID,UserName,UserSex,Switch (UserState = 1 , '正常' ,UserState = 0 , '禁止' ) as UserState,CreateDate as CreateDate from UserInfo where UserID in (select a.UserID from GroupUser a left join [Group] b on a.GroupID=b.GroupID where b.UserID="+Convert.ToInt16(myUserID)+")  order by UserID desc";
		...
		private void ShowData(string strSql)
		{
			...
			{
                HyperLink LinkButTestLog = (HyperLink)DataGridUser.Items[i].FindControl("LinkButTestLog");
                HyperLink LinkButLore = (HyperLink)DataGridUser.Items[i].FindControl("LinkButLore");
                
			
                 LinkButTestLog.NavigateUrl = "AnswerLog.aspx?UserID="+DataGridUser.Items[i].Cells[0].Text+"";
                 LinkButTestLog.Target = "_blank";

                 LinkButLore.NavigateUrl = "UserLoreList.aspx?UserID="+DataGridUser.Items[i].Cells[0].Text;
