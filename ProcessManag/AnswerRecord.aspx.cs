	...
	public partial class AnswerRecord : System.Web.UI.Page
	{
		...
		protected void Page_Load(object sender, System.EventArgs e)
		{
			...
			if (!IsPostBack)
			{
                ...
                if (AccessDateHelper.GetValues("select UserType from UserInfo where LoginID='" + myLoginID + "' and UserType=1 and (RoleMenu=1 or (RoleMenu=2 and Exists(select OptionID from UserPower where UserID=" + UserID + " and PowerID=3 and OptionID=5)))", "UserType") != "1")
				{
		...
		private void ShowData(string strSql)
		{
			...
			DataGridScore.DataSource=SqlDS.Tables[0].DefaultView;
		...
		protected void ButDelete_Click(object sender, System.EventArgs e)
		{
			if (Request["chkSelect"]!=null)
			{
				...
				for (int j=0;j<textArray.Length;j++)
				{
                    ...
                    AccessDateHelper.ExecuteNonQuery("delete from UserAnswer where UserScoreID=" + DataGridScore.Items[i].Cells[0].Text.Trim());
                    AccessDateHelper.ExecuteNonQuery("delete from UserScore where UserScoreID=" + DataGridScore.Items[i].Cells[0].Text.Trim());
				}
		...
		private void DataGridScore_DeleteCommand(object source, System.Web.UI.WebControls.DataGridCommandEventArgs e)
		{
            ...
            AccessDateHelper.ExecuteNonQuery("delete from UserAnswer where UserScoreID=" + intUserScoreID + "");
            AccessDateHelper.ExecuteNonQuery("delete from UserScore where UserScoreID=" + intUserScoreID + "");
		...
		{
			bWhere=true;
            strSql = "select a.UserScoreID,b.UserID,b.LoginID,b.UserName,c.DeptName,round(a.TotalMark,1) as TotalMark,a.StartTime,a.EndTime,a.LoginIP,switch( a.ExamState = 0 , '答卷中' ,a.ExamState = 1 ,a.ExamState = '已交卷' ) as ExamState,d.LoginID as JudgeLoginID,a.ImpScore from UserScore a LEFT  JOIN UserInfo b ON a.UserID=b.UserID LEFT  JOIN DeptInfo c ON b.DeptID=c.DeptID LEFT  JOIN UserInfo d ON a.JudgeUserID=d.UserID,UserInfo e where a.PaperID=" + intPaperID + " and e.UserID=" + intUserID + " and (e.JudgeUser=1 or (e.JudgeUser=2 and Exists(select * from UserPower where UserPower.UserID=" + intUserID + " and UserPower.PowerID=1 and UserPower.OptionID=a.UserID)) or Exists(select * from UserInfo f,DeptInfo g,UserPower h where f.UserID=a.UserID and f.DeptID=g.DeptID and g.DeptID=h.DeptID and h.UserID=" + intUserID + " and h.PowerID=1))";
			//显示以部门为条件的数据
