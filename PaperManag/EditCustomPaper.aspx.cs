	...
	public partial class EditCustomPaper : System.Web.UI.Page
	{
		...
		protected void Page_Load(object sender, System.EventArgs e)
		{
			...
			if (!IsPostBack)
			{
                ...
                if (AccessDateHelper.GetValues("select UserType from UserInfo where LoginID='" + myLoginID + "' and UserType=1 and (RoleMenu=1 or (RoleMenu=2 and Exists(select OptionID from UserPower where UserID=" + UserID + " and PowerID=3 and OptionID=4)))", "UserType") != "1")
				{
		...
		private void LoadPaperData()
		{
			...
			if (ObjDR.Read())
			{
				...
				if (ObjDR["ExamAccount"].ToString()=="2")
				{
                    ...
                    SqlDS = AccessDateHelper.ExecuteDataset("select b.DeptID,b.DeptName from PaperUser a,DeptInfo b where a.DeptID=b.DeptID and a.UserType=0 and a.PaperID=" + intPaperID + " order by a.DeptID asc");
					for(i=0;i<SqlDS.Tables[0].Rows.Count;i++)
					{
						if (txtSelectExam.Text.Trim()=="")
						{
							txtSelectExam.Text=txtSelectExam.Text+SqlDS.Tables[0].Rows[i]["DeptName"].ToString();
						}
						else
						{
							txtSelectExam.Text=txtSelectExam.Text+";"+SqlDS.Tables[0].Rows[i]["DeptName"].ToString();
                    ...
                    SqlDS = AccessDateHelper.ExecuteDataset("select b.UserID,b.LoginID from PaperUser a,UserInfo b where a.UserID=b.UserID and a.UserType=0 and a.PaperID=" + intPaperID + " order by a.UserID asc");
					for(i=0;i<SqlDS.Tables[0].Rows.Count;i++)
					{
						if (txtSelectExam.Text.Trim()=="")
						{
							txtSelectExam.Text=txtSelectExam.Text+SqlDS.Tables[0].Rows[i]["LoginID"].ToString();
						}
						else
						{
							txtSelectExam.Text=txtSelectExam.Text+";"+SqlDS.Tables[0].Rows[i]["LoginID"].ToString();
				...
				if (ObjDR["ManagerAccount"].ToString()=="2")
				{
                    ...
                    //SqlDS=new DataSet();
                    //SqlCmd.Fill(SqlDS,"DeptInfo");
                    SqlDS = AccessDateHelper.ExecuteDataset("select b.DeptID,b.DeptName from PaperUser a,DeptInfo b where a.DeptID=b.DeptID and a.UserType=1 and a.PaperID=" + intPaperID + " order by a.DeptID asc");
					for(i=0;i<SqlDS.Tables[0].Rows.Count;i++)
					{
						if (txtSelectManager.Text.Trim()=="")
						{
							txtSelectManager.Text=txtSelectManager.Text+SqlDS.Tables[0].Rows[i]["DeptName"].ToString();
						}
						else
						{
							txtSelectManager.Text=txtSelectManager.Text+";"+SqlDS.Tables[0].Rows[i]["DeptName"].ToString();
                    ...
                    //SqlDS=new DataSet();
                    //SqlCmd.Fill(SqlDS,"UserInfo");
                    SqlDS = AccessDateHelper.ExecuteDataset("select b.UserID,b.LoginID from PaperUser a,UserInfo b where a.UserID=b.UserID and a.UserType=1 and a.PaperID=" + intPaperID + " order by a.UserID asc");
					for(i=0;i<SqlDS.Tables[0].Rows.Count;i++)
					{
						if (txtSelectManager.Text.Trim()=="")
						{
							txtSelectManager.Text=txtSelectManager.Text+SqlDS.Tables[0].Rows[i]["LoginID"].ToString();
						}
						else
						{
							txtSelectManager.Text=txtSelectManager.Text+";"+SqlDS.Tables[0].Rows[i]["LoginID"].ToString();
			...
			else
			{
				...
				try
				{
					...
					if (rbMarkDefine1.Checked==true)
					{
                        ...
                        //SqlDS=new DataSet();
                        //SqlCmd.Fill(SqlDS,"PaperTest");
                        SqlDS = AccessDateHelper.ExecuteDataset("select sum(TestMark) as TotalMark from PaperTest where PaperID=" + intPaperID + "");
						dblTotalMark=Convert.ToDouble(SqlDS.Tables[0].Rows[0]["TotalMark"]);
                        ...
                        //ObjectCmd.ExecuteNonQuery();
                        AccessDateHelper.ExecuteNonQuery("Update PaperTest set TestMark=Round(TestMark*" + dblRate + ",2) where PaperID=" + intPaperID + "");
					}
					if (rbMarkDefine2.Checked==true)
					{
                        ...
                        //SqlDS=new DataSet();
                        //SqlCmd.Fill(SqlDS,"PaperTestType");
                        SqlDS= AccessDateHelper.ExecuteDataset("select * from PaperTestType where PaperID="+intPaperID+"");
						for(i=0;i<SqlDS.Tables[0].Rows.Count;i++)
						{
							//SqlCmdTmp=new SqlDataAdapter("select a.PaperTestID,b.TestTypeID from PaperTest a,RubricInfo b where a.RubricID=b.RubricID and b.TestTypeID="+SqlDS.Tables[0].Rows[i]["TestTypeID"]+" and a.PaperID="+intPaperID+"",SqlConn);
							//SqlDSTmp=new DataSet();
							//SqlCmdTmp.Fill(SqlDSTmp,"PaperTest");
							//for(j=0;j<SqlDSTmp.Tables[0].Rows.Count;j++)
							//{
							//	ObjectCmd.CommandText="Update PaperTest set TestMark="+SqlDS.Tables[0].Rows[i]["TestTypeMark"]+" where PaperTestID='"+SqlDSTmp.Tables[0].Rows[j]["PaperTestID"]+"'";
							//	ObjectCmd.ExecuteNonQuery();
							//}

                            //ObjectCmd.CommandText="Update PaperTest set TestMark="+SqlDS.Tables[0].Rows[i]["TestTypeMark"]+" from PaperTest,RubricInfo where PaperTest.RubricID=RubricInfo.RubricID and RubricInfo.TestTypeID="+SqlDS.Tables[0].Rows[i]["TestTypeID"]+" and PaperTest.PaperID="+intPaperID+"";
                            //ObjectCmd.ExecuteNonQuery();
                            AccessDateHelper.ExecuteNonQuery("Update PaperTest set TestMark="+SqlDS.Tables[0].Rows[i]["TestTypeMark"]+" from PaperTest,RubricInfo where PaperTest.RubricID=RubricInfo.RubricID and RubricInfo.TestTypeID="+SqlDS.Tables[0].Rows[i]["TestTypeID"]+" and PaperTest.PaperID="+intPaperID+"");
						}
                    ...
                    //SqlDS=new DataSet();
                    //SqlCmd.Fill(SqlDS,"PaperTestType");
                    AccessDateHelper.ExecuteDataset("select TestTypeID from PaperTestType where PaperID="+intPaperID+" order by PaperTestTypeID asc");
					for(i=0;i<SqlDS.Tables[0].Rows.Count;i++)
					{
                    ...
                    AccessDateHelper.ExecuteNonQuery("Update PaperInfo set PaperName='"+strPaperName+"',PaperType='"+intPaperType+"',ProduceWay='"+intProduceWay+"',ShowModal='"+intShowModal+"',ExamTime='"+intExamTime+"',StartTime='"+dtmStartTime+"',EndTime='"+dtmEndTime+"',PaperMark='"+intPaperMark+"',PassMark='"+intPassMark+"',MarkDefine='"+intMarkDefine+"',RepeatExam='"+intRepeatExam+"',FillAutoGrade='"+intFillAutoGrade+"',SeeResult='"+intSeeResult+"',AutoSave='"+intAutoSave+"',ExamAccount='"+intExamAccount+"',ManagerAccount='"+intManagerAccount+"',TestCount='"+intTestCount+"',AutoJudge='"+intAutoJudge+"',CreateWay=2 where PaperID="+intPaperID+"");
                    ...
                    //ObjectCmd.ExecuteNonQuery();
                    AccessDateHelper.ExecuteNonQuery("delete UserAnswer from UserAnswer a LEFT  JOIN UserScore b ON a.UserScoreID=b.UserScoreID where b.PaperID="+intPaperID+" and b.ExamState<=0");
					...
					for(i=0;i<SqlDS.Tables[0].Rows.Count;i++)
					{
                        ...
                        //AccessDateHelper.ExecuteNonQuery("update UserScore set UserID="+intUserID+",PaperID="+intPaperID+",ExamState=-1，LoginIP='"+Convert.ToString(Request.ServerVariables["Remote_Addr"])+"'");
					}
