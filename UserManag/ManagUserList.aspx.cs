...
namespace EasyExam.UserManag
{
	...
	public partial class ManagUserList : System.Web.UI.Page
	{
		...
		{			
			...
			if (!IsPostBack)
			{
				...
				else
				{
                    ...
                    strSql = "select a.UserID,a.LoginID,a.UserName,a.UserSex,b.DeptName,c.JobName,Switch( a.UserType = 0 , '普通帐户' , a.UserType =1 , '管理帐户' ) as UserType,Switch (a.UserState = 1 , '正常' ,a.UserState = 0 , '禁止' ) as UserState,d.LoginID as CreateLoginID,a.CreateDate as CreateDate from ((UserInfo a LEFT  JOIN DeptInfo b ON a.DeptID=b.DeptID) LEFT  JOIN JobInfo c ON a.JobID=c.JobID) LEFT  JOIN UserInfo d ON a.CreateUserID=d.UserID order by a.UserID desc";
		...
		private void ShowData(string strSql)
		{
			...
			{
                HyperLink LBEditUser = (HyperLink)DataGridUser.Items[i].FindControl("LinkButEditUser");
				...
				if ((myLoginID.Trim().ToUpper()=="ADMIN")||(myLoginID.Trim().ToUpper()==DataGridUser.Items[i].Cells[10].Text.Trim().ToUpper()))
				{
                    ...
                    LBEditUser.NavigateUrl = "EditOneUser.aspx?UserID="+DataGridUser.Items[i].Cells[0].Text;
                    LBEditUser.Target="main";
				...
				for (int j=0;j<textArray.Length;j++)
			    {
					...
					if ((DataGridUser.Items[i].Cells[3].Text.Trim().ToUpper()!="ADMIN")&&(DataGridUser.Items[i].Cells[3].Text.Trim().ToUpper()!="GUEST"))
					{
						if ((myLoginID.Trim().ToUpper()=="ADMIN")||(myLoginID.Trim().ToUpper()==DataGridUser.Items[i].Cells[10].Text.Trim().ToUpper()))
						{
                            ...
                            AccessDateHelper.ExecuteNonQuery("delete from NewsUser where UserID=" + DataGridUser.Items[i].Cells[0].Text.Trim() + "");
							//数据表PaperUser
                            //SqlCmd=new SqlCommand("delete from PaperUser where UserID="+DataGridUser.Items[i].Cells[0].Text.Trim()+"",SqlConn);
                            //SqlCmd.ExecuteNonQuery();

                            AccessDateHelper.ExecuteNonQuery("delete from PaperUser where UserID=" + DataGridUser.Items[i].Cells[0].Text.Trim() + "");
							//数据表UserAnswer
                            ...
                            //SqlCmd.ExecuteNonQuery();
                            string UserScoreID = AccessDateHelper.GetValues("select UserScoreID from UserScore where UserID=" + DataGridUser.Items[i].Cells[0].Text.Trim(), "UserScoreID");
                            AccessDateHelper.ExecuteNonQuery("delete from UserAnswer  where UserScoreID=" + UserScoreID + "");
                           
                            ////数据表UserScore
                            //SqlCmd=new SqlCommand("delete from UserScore where UserID="+DataGridUser.Items[i].Cells[0].Text.Trim()+"",SqlConn);
                            //SqlCmd.ExecuteNonQuery();
                            AccessDateHelper.ExecuteNonQuery("delete from UserScore where UserID=" + DataGridUser.Items[i].Cells[0].Text.Trim() + "");
                            ////数据表UserPower
                            //SqlCmd=new SqlCommand("delete from UserPower where UserID="+DataGridUser.Items[i].Cells[0].Text.Trim()+"",SqlConn);
                            //SqlCmd.ExecuteNonQuery();
                            AccessDateHelper.ExecuteNonQuery("delete from UserPower where UserID=" + DataGridUser.Items[i].Cells[0].Text.Trim() + "");
                            //SqlCmd=new SqlCommand("delete from UserPower where PowerID=1 and OptionID="+DataGridUser.Items[i].Cells[0].Text.Trim()+"",SqlConn);
                            //SqlCmd.ExecuteNonQuery();

                            AccessDateHelper.ExecuteNonQuery("delete from UserPower where PowerID=1 and OptionID=" + DataGridUser.Items[i].Cells[0].Text.Trim() + "");
                            ////数据表UserInfo
                            //SqlCmd=new SqlCommand("delete from UserInfo where UserID="+DataGridUser.Items[i].Cells[0].Text.Trim()+"",SqlConn);
                            //SqlCmd.ExecuteNonQuery();

                            AccessDateHelper.ExecuteNonQuery("delete from UserInfo where UserID=" + DataGridUser.Items[i].Cells[0].Text.Trim() + "");
						}
					}
				}
		...
		{
			bWhere=false;
            strSql = "select a.UserID,a.LoginID,a.UserName,a.UserSex,b.DeptName,c.JobName,Switch( a.UserType = 0 , '学生账户' , a.UserType =1 , '管理帐户',a.UserType=2,'老师账户' ) as UserType,Switch (a.UserState = 1 , '正常' ,a.UserState = 0 , '禁止' ) as UserState,d.LoginID as CreateLoginID,a.CreateDate as CreateDate from ((UserInfo a LEFT  JOIN DeptInfo b ON a.DeptID=b.DeptID )LEFT  JOIN JobInfo c ON a.JobID=c.JobID )LEFT  JOIN UserInfo d ON a.CreateUserID=d.UserID";
			//显示以部门为条件的数据
			...
			if (txtLoginID.Text.Trim()!="")
			{
				if (bWhere)
				{
					strSql=strSql+" and a.LoginID like '%"+ObjFun.CheckString(txtLoginID.Text.Trim())+"%'";
				}
				else
				{
					strSql=strSql+" where a.LoginID like '%"+ObjFun.CheckString(txtLoginID.Text.Trim())+"%'";
					bWhere=true;
				}
			}
			//显示以姓名为条件的数据
				...
				for (int j=0;j<textArray.Length;j++)
				{
					...
					if ((DataGridUser.Items[i].Cells[3].Text.Trim().ToUpper()!="ADMIN")&&(DataGridUser.Items[i].Cells[3].Text.Trim().ToUpper()!="GUEST"))
					{
						if ((myLoginID.Trim().ToUpper()=="ADMIN")||(myLoginID.Trim().ToUpper()==DataGridUser.Items[i].Cells[10].Text.Trim().ToUpper()))
						{
                            ...
                            AccessDateHelper.ExecuteNonQuery("update UserInfo set UserState=0 where UserID=" + DataGridUser.Items[i].Cells[0].Text.Trim());
						}
					}
				}
				//SqlConn.Close();
			...
			if (Request["chkSelect"]!=null)
			{
				...
				for (int j=0;j<textArray.Length;j++)
				{
					...
					if ((myLoginID.Trim().ToUpper()=="ADMIN")||(myLoginID.Trim().ToUpper()==DataGridUser.Items[i].Cells[10].Text.Trim().ToUpper()))
					{
                        ...
                        //SqlCmd.ExecuteNonQuery();
                        AccessDateHelper.ExecuteNonQuery("update UserInfo set UserState=1 where UserID=" + DataGridUser.Items[i].Cells[0].Text.Trim());
					}
				}
				//SqlConn.Close();
				ShowData(strSql);//显示数据
			...
			if (Request["chkSelect"]!=null)
			{
				...
				for (int j=0;j<textArray.Length;j++)
				{
					...
					if ((myLoginID.Trim().ToUpper()=="ADMIN")||(myLoginID.Trim().ToUpper()==DataGridUser.Items[i].Cells[10].Text.Trim().ToUpper()))
					{
                        ...
                        //SqlCmd.ExecuteNonQuery();
                        AccessDateHelper.ExecuteNonQuery("update UserInfo set UserPwd='' where UserID=" + DataGridUser.Items[i].Cells[0].Text.Trim());
					}
				}
				//SqlConn.Close();
				ShowData(strSql);//显示数据
			...
			if (Request["chkSelect"]!=null)
			{
				...
				for (int j=0;j<textArray.Length;j++)
				{
					...
					if ((myLoginID.Trim().ToUpper()=="ADMIN")||(myLoginID.Trim().ToUpper()==DataGridUser.Items[i].Cells[10].Text.Trim().ToUpper()))
					{
                        ...
                        //SqlCmd=new SqlCommand("delete UserAnswer from UserAnswer a LEFT OUTER JOIN UserScore b ON a.UserScoreID=b.UserScoreID where b.UserID="+DataGridUser.Items[i].Cells[0].Text.Trim()+"",SqlConn);
                        //SqlCmd.ExecuteNonQuery();
                        AccessDateHelper.ExecuteNonQuery("delete UserAnswer from UserAnswer a LEFT OUTER JOIN UserScore b ON a.UserScoreID=b.UserScoreID where b.UserID=" + DataGridUser.Items[i].Cells[0].Text.Trim() + "");
						//数据表UserScore
                        //SqlCmd=new SqlCommand("delete from UserScore where UserID="+DataGridUser.Items[i].Cells[0].Text.Trim()+"",SqlConn);
                        //SqlCmd.ExecuteNonQuery();
