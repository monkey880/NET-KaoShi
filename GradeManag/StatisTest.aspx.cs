...
namespace EasyExam.GradeManag
{
	...
	public partial class StatisTest : System.Web.UI.Page
	{
		...
		protected void Page_Load(object sender, System.EventArgs e)
		{
            ...
            intCount = Convert.ToInt32(AccessDateHelper.GetValues("select count(*) as count2 from UserScore where PaperID=" + intPaperID + " and ExamState=1", "count2"));//答卷记录数
		...
		private void ShowData(string strSql)
		{
			...
			for(int i=0;i<DataGridTest.Items.Count;i++)
			{
				...
				if (DataGridTest.Items[i].Cells[3].Text.Trim()=="单选类")
				{
                    ...
                    SqlDSTmp = AccessDateHelper.ExecuteDataset("select a.RubricID,a.UserAnswer from UserAnswer a,UserScore b where a.UserScoreID=b.UserScoreID and a.RubricID=" + DataGridTest.Items[i].Cells[0].Text.Trim() + " and b.PaperID=" + intPaperID + " and b.ExamState=1");
					for(j=0;j<SqlDSTmp.Tables[0].Rows.Count;j++)
					{
						if (SqlDSTmp.Tables[0].Rows[j]["UserAnswer"].ToString().ToUpper()=="A")
						{
							intSelA=intSelA+1;
						}
						if (SqlDSTmp.Tables[0].Rows[j]["UserAnswer"].ToString().ToUpper()=="B")
						{
							intSelB=intSelB+1;
						}
						if (SqlDSTmp.Tables[0].Rows[j]["UserAnswer"].ToString().ToUpper()=="C")
						{
							intSelC=intSelC+1;
						}
						if (SqlDSTmp.Tables[0].Rows[j]["UserAnswer"].ToString().ToUpper()=="D")
						{
							intSelD=intSelD+1;
						}
						if (SqlDSTmp.Tables[0].Rows[j]["UserAnswer"].ToString().ToUpper()=="E")
						{
							intSelE=intSelE+1;
						}
						if (SqlDSTmp.Tables[0].Rows[j]["UserAnswer"].ToString().ToUpper()=="F")
						{
							intSelF=intSelF+1;
						}
					}
					Label labSelA=(Label)DataGridTest.Items[i].FindControl("labSelA");
					labSelA.Text=Convert.ToString(System.Math.Round((Convert.ToDouble(intSelA)/SqlDSTmp.Tables[0].Rows.Count)*100,1))+"%";

					Label labSelB=(Label)DataGridTest.Items[i].FindControl("labSelB");
					labSelB.Text=Convert.ToString(System.Math.Round((Convert.ToDouble(intSelB)/SqlDSTmp.Tables[0].Rows.Count)*100,1))+"%";

					Label labSelC=(Label)DataGridTest.Items[i].FindControl("labSelC");
					labSelC.Text=Convert.ToString(System.Math.Round((Convert.ToDouble(intSelC)/SqlDSTmp.Tables[0].Rows.Count)*100,1))+"%";

					Label labSelD=(Label)DataGridTest.Items[i].FindControl("labSelD");
					labSelD.Text=Convert.ToString(System.Math.Round((Convert.ToDouble(intSelD)/SqlDSTmp.Tables[0].Rows.Count)*100,1))+"%";

					Label labSelE=(Label)DataGridTest.Items[i].FindControl("labSelE");
					labSelE.Text=Convert.ToString(System.Math.Round((Convert.ToDouble(intSelE)/SqlDSTmp.Tables[0].Rows.Count)*100,1))+"%";

					Label labSelF=(Label)DataGridTest.Items[i].FindControl("labSelF");
					labSelF.Text=Convert.ToString(System.Math.Round((Convert.ToDouble(intSelF)/SqlDSTmp.Tables[0].Rows.Count)*100,1))+"%";
				}
				if (DataGridTest.Items[i].Cells[3].Text.Trim()=="多选类")
				{
					...
					for(j=0;j<SqlDSTmp.Tables[0].Rows.Count;j++)
					{
						if (SqlDSTmp.Tables[0].Rows[j]["UserAnswer"].ToString().ToUpper().IndexOf("A")>=0)
						{
							intSelA=intSelA+1;
						}
						if (SqlDSTmp.Tables[0].Rows[j]["UserAnswer"].ToString().ToUpper().IndexOf("B")>=0)
						{
							intSelB=intSelB+1;
						}
						if (SqlDSTmp.Tables[0].Rows[j]["UserAnswer"].ToString().ToUpper().IndexOf("C")>=0)
						{
							intSelC=intSelC+1;
						}
						if (SqlDSTmp.Tables[0].Rows[j]["UserAnswer"].ToString().ToUpper().IndexOf("D")>=0)
						{
							intSelD=intSelD+1;
						}
						if (SqlDSTmp.Tables[0].Rows[j]["UserAnswer"].ToString().ToUpper().IndexOf("E")>=0)
						{
							intSelE=intSelE+1;
						}
						if (SqlDSTmp.Tables[0].Rows[j]["UserAnswer"].ToString().ToUpper().IndexOf("F")>=0)
						{
							intSelF=intSelF+1;
						}
					}
