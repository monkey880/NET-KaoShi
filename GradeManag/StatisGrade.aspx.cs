...
//			}
			...
			if (!IsPostBack)
			{
				...
				else
				{
					if (intPaperID!=0)
					{
                        strSql = "select a.UserScoreID,b.LoginID,b.UserName,b.UserSex,c.DeptName,d.JobName,switch( b.UserType = 1 , '管理帐户' ,b.UserType = 0 , '普通帐户' ) as UserType,switch( b.UserState = 1 , '正常' ,b.UserState = 0 , '禁止' ) as UserState,round(a.TotalMark,1) as TotalMark from JobInfo d  RIGHT JOIN (DeptInfo c RIGHT JOIN (UserInfo b RIGHT JOIN UserScore a ON b.UserID = a.UserID) ON c.DeptID = b.DeptID) ON d.JobID = b.JobID where a.PaperID=" + intPaperID + " and a.UserID=b.UserID and a.ExamState=1 order by a.TotalMark desc";
		...
		{		
			bWhere=true;
            strSql = "select a.UserScoreID,b.LoginID,b.UserName,b.UserSex,c.DeptName,d.JobName,switch( b.UserType = 1 , '管理帐户' ,b.UserType = 0 , '普通帐户' ) as UserType,switch( b.UserState = 1 , '正常' ,b.UserState = 0 , '禁止' ) as UserState,round(a.TotalMark,1) as TotalMark from (UserScore a,UserInfo b LEFT  JOIN DeptInfo c ON b.DeptID=c.DeptID) LEFT  JOIN JobInfo d ON b.JobID=d.JobID where a.PaperID=" + intPaperID + " and a.UserID=b.UserID and a.ExamState=1";
			//显示以部门为条件的数据
