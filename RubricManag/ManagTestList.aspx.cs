...
namespace EasyExam.RubricManag
{
	...
	public partial class ManagTestList : System.Web.UI.Page
	{
		...
		protected void Page_Load(object sender, System.EventArgs e)
		{
			...
			if (!IsPostBack)
			{
				...
				else
				{
                    ...
                    ButNewTest.Attributes.Add("onclick", "javascript:window.location.href='NewTest.aspx'; return false;");
					ButDelete.Attributes.Add("onclick","javascript:{if(confirm('确实要删除选择试题吗？')==false) return false;}");
                    strSql = "select a.RubricID,b.SubjectName,c.LoreName,d.TestTypeName,a.TestContent,a.OptionContent,a.OptionNum,e.LoginID as CreateLoginID,a.CreateDate as CreateDate from (((RubricInfo a LEFT  JOIN SubjectInfo b ON a.SubjectID=b.SubjectID) LEFT  JOIN LoreInfo c ON a.LoreID=c.LoreID )LEFT  JOIN TestTypeInfo d ON a.TestTypeID=d.TestTypeID )LEFT  JOIN UserInfo e ON a.CreateUserID=e.UserID order by a.RubricID desc";
		...
		private void DataGridTest_DeleteCommand(object source, System.Web.UI.WebControls.DataGridCommandEventArgs e)
		{
			...
			{
                if (AccessDateHelper.GetValues("select PaperTestID from PaperTest where RubricID=" + intRubricID + "", "PaperTestID") == "")
				{
                    //string strConn=ConfigurationSettings.AppSettings["strConn"];
                    //SqlConnection SqlConn=new SqlConnection(strConn);
                    ...
                    AccessDateHelper.ExecuteNonQuery("delete from RubricInfo where RubricID=" + intRubricID + "");
		...
		private void ShowData(string strSql)
		{
			...
			DataGridTest.DataSource=SqlDS.Tables[0].DefaultView;
			...
			for(int i=0;i<DataGridTest.Items.Count;i++)
			{
                ...
                strPaperContent = strPaperContent + "<ul class='list-group' id='trTestTypeContent" + Convert.ToString(i + 1) + "'>";
                 intTestNum = intTestNum + 1;
                string strTestContent = DataGridTest.Items[i].Cells[1].Text;

                if (DataGridTest.Items[i].Cells[5].Text == "真空题")
                {
                    int intOptionNum = 0;
                    ...
                    strTestContent = DataGridTest.Items[i].Cells[1].Text;

                    for (int k = 1; k <= intOptionNum; k++)
                    {

                        strTestContent = Reg.Replace(strTestContent, "<input type='text' id='Answer" + DataGridTest.Items[i].Cells[0].Text + "' name='Answer" + DataGridTest.Items[i].Cells[0].Text + "' size='16' class=filltext value='' onBlur='textcheck()' title='试题答案中不能包含半角逗号“,”'>", 1);
                        strPaperContent = strPaperContent + strTestContent;
                    }
                }
              

                if (DataGridTest.Items[i].Cells[5].Text == "单选题" || DataGridTest.Items[i].Cells[5].Text == "多选题")
                {
                    int intOptionNum = Convert.ToInt32(DataGridTest.Items[i].Cells[2].Text);
                    string[] strArrOptionContent = DataGridTest.Items[i].Cells[1].Text.Split('|');
                    for (int k = 1; k <= intOptionNum; k++)
                    {


                        strPaperContent = strPaperContent + "<li class='list-group-item'>" + Convert.ToChar(64 + k) + "." + strArrOptionContent[k - 1] + "</li>";
                ...
                Label TestOption = (Label)DataGridTest.Items[i].FindControl("TestOption");
                TestOption.Text = strPaperContent;
		...
		protected void ButDelete_Click(object sender, System.EventArgs e)
		{
			if (Request["chkSelect"]!=null)
			{
				...
				for (int j=0;j<textArray.Length;j++)
				{
					...
					if ((myLoginID.Trim().ToUpper()=="ADMIN")||(myLoginID.Trim().ToUpper()==DataGridTest.Items[i].Cells[7].Text.Trim().ToUpper()))
					{
                        if (AccessDateHelper.GetValues("select PaperTestID from PaperTest where RubricID=" + DataGridTest.Items[i].Cells[0].Text.Trim() + "", "PaperTestID") == "")
						{
                            //SqlCmd=new SqlCommand("delete from RubricInfo where RubricID="+DataGridTest.Items[i].Cells[0].Text.Trim(),SqlConn);
                            //SqlCmd.ExecuteNonQuery();
                            AccessDateHelper.ExecuteNonQuery("delete from RubricInfo where RubricID=" + DataGridTest.Items[i].Cells[0].Text.Trim());
						}
					}
		...
		{
			bWhere=false;
			strSql="select a.RubricID,b.SubjectName,c.LoreName,d.TestTypeName,a.TestContent,e.LoginID as CreateLoginID,a.CreateDate as CreateDate from (((RubricInfo a LEFT  JOIN SubjectInfo b ON a.SubjectID=b.SubjectID) LEFT  JOIN LoreInfo c ON a.LoreID=c.LoreID) LEFT  JOIN TestTypeInfo d ON a.TestTypeID=d.TestTypeID) LEFT  JOIN UserInfo e ON a.CreateUserID=e.UserID";
			//显示以科目为条件的数据
		...
		protected void ButExport_Click(object sender, System.EventArgs e)
		{
			...
			{
				bWhere=false;
				strSql="select b.SubjectName,c.LoreName,d.TestTypeName,a.TestDiff,a.TestMark,a.TestContent,a.OptionContent,a.StandardAnswer,a.TestParse from ((RubricInfo a LEFT  JOIN SubjectInfo b ON a.SubjectID=b.SubjectID) LEFT  JOIN LoreInfo c ON a.LoreID=c.LoreID) LEFT  JOIN TestTypeInfo d ON a.TestTypeID=d.TestTypeID";
			}
			else
			{
				bWhere=true;
				strSql="select b.SubjectName,c.LoreName,d.TestTypeName,a.TestDiff,a.TestMark,a.TestContent,a.OptionContent,a.StandardAnswer,a.TestParse from ((RubricInfo a LEFT  JOIN SubjectInfo b ON a.SubjectID=b.SubjectID )LEFT  JOIN LoreInfo c ON a.LoreID=c.LoreID) LEFT  JOIN TestTypeInfo d ON a.TestTypeID=d.TestTypeID where a.CreateUserID="+Convert.ToInt32(myUserID)+"";
			}
			//显示以科目为条件的数据
			if (DDLSubjectName.SelectedItem.Value!="0")
			{
